...
async function buildCodexHeader() {
  ...
  const { installed, hasHiperRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasHiperRouter, settings } = result.data;
  ...
  // Extract HiperRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:HiperRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract HiperRouter provider config
  const provider = settings?.models?.providers?.["hiperrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("hiperrouter/") ? primary.replace("hiperrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasHiperRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use HiperRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
