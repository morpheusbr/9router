# HiperRouter Architecture

_Last updated: 2026-02-06_

## Executive Summary

HiperRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[HiperRouter Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.HiperRouter/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.HiperRouter/usage.json`, `~/.HiperRouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.HiperRouter/db.json`)
- usage stats: `~/.HiperRouter/usage.json`
- request log lines: `~/.HiperRouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[HiperRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.HiperRouter` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/hiperrouter && npm run build`
- Build Docker image: `cd /root/dev/hiperrouter && docker build -t hiperrouter .`
