...
const hasHiperRouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "HiperRouter");
};

const getHiperRouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "HiperRouter") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getHiperRouterEntry(config);
...
// POST - Apply HiperRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing HiperRouter entry or append
    const idx = config.findIndex((e) => e.name === "HiperRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove HiperRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "HiperRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "HiperRouter removed from Copilot config",
    });
  } catch (error) {
