...
const hasHiperRouterConfig = (config) => {
  ...
  return !!config.provider["hiperrouter"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["hiperrouter"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasHiperRouter: hasHiperRouterConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("hiperrouter/") ? config.model.replace(/^hiperrouter\//, "") : null,
...
// POST - Apply HiperRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_hiperrouter";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing hiperrouter provider entry and its models
    const existingProvider = config.provider["hiperrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["hiperrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `hiperrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `hiperrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("hiperrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove HiperRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["hiperrouter"]?.models) {
      delete config.provider["hiperrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["hiperrouter"].models).length === 0) {
        delete config.provider["hiperrouter"];
        if (config.model?.startsWith("hiperrouter/")) delete config.model;
      } else if (config.model === `hiperrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["hiperrouter"].models);
        config.model = `hiperrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire hiperrouter provider
      if (config.provider) delete config.provider["hiperrouter"];
      if (config.model?.startsWith("hiperrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("hiperrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "HiperRouter settings removed from OpenCode",
    });
  } catch (error) {
