...
const hasHiperRouterConfig = (settings) => {
  ...
  return !!settings.models.providers["hiperrouter"];
};

// Read per-agent models.json and return current model id (without "hiperrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["hiperrouter"]?.models;
...
const writeAgentModels = async (agentDir, model, baseUrl, apiKey) => {
  ...
  existing.providers["hiperrouter"] = {
    baseUrl,
...
export async function POST(request) {
  try {
    ...
    const fullModelId = `hiperrouter/${model}`;

    // Remove all old hiperrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("hiperrouter/"))
    ...
    // Add fresh hiperrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`hiperrouter/${m}`] = {};
    });

    // Remove old hiperrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("hiperrouter/")) {
    ...
    settings.models.providers["hiperrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `hiperrouter/${agentModel}` };
        return agent;
      });
...
export async function DELETE() {
  try {
    ...
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["hiperrouter"];
    ...
    // Remove hiperrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("hiperrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses hiperrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("hiperrouter/")) {
