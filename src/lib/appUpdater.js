  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "hiperrouter")
        : path.join(os.homedir(), ".HiperRouter"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all hiperrouter-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from hiperrouter install dir or wrapper cli.js
        const isAppProcess = lower.includes("hiperrouter") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("hiperrouter") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "hiperrouter");
  }
  return path.join(os.homedir(), ".HiperRouter");
}
...
// Resolve npx/hiperrouter binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx hiperrouter` — works regardless of global bin path changes after npm i -g
