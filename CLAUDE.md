...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.HiperRouter/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.HiperRouter` and do **not** follow `DATA_DIR`.
