...
Tích hợp HiperRouter với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của HiperRouter.
...
HiperRouter cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong HiperRouter dashboard
...
1. Xác minh tên model khớp với cấu hình HiperRouter
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong HiperRouter dashboard
...
Đảm bảo bạn đã cấu hình API key trong HiperRouter cloud dashboard.
