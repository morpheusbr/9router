...
Tích hợp HiperRouter với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của HiperRouter.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- HiperRouter đang chạy cục bộ hoặc cloud endpoint đã cấu hình
...
Đặt base URL tới endpoint HiperRouter:

**Cho HiperRouter cục bộ:**
```
http://localhost:20128/v1
```

**Cho HiperRouter cloud:**
```
https://9router.com
```

**Các bước:**
1. Trong field **Base URL**, nhập endpoint HiperRouter
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key HiperRouter của bạn
...
1. Trong dropdown **Model**, bạn có thể:
   ...
   - Nhập tên model thủ công từ cấu hình HiperRouter
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng HiperRouter để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua HiperRouter
...
3. Nhận giải thích AI qua HiperRouter

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng HiperRouter để hiểu context và thực hiện thay đổi
...
1. Xác minh HiperRouter đang chạy: `curl http://localhost:20128/health`
...
2. Xác minh HiperRouter instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs HiperRouter để xem lỗi

## Cấu hình Nâng cao

### Dùng Cloud Endpoint

Để dùng HiperRouter cloud endpoint thay vì localhost:

1. Trong Cline settings, đặt Base URL: `https://9router.com`
2. Đảm bảo bạn đã cấu hình API key trong HiperRouter cloud dashboard
...
## Tích hợp với Tính năng HiperRouter

### Định tuyến Model

HiperRouter tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, HiperRouter tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua HiperRouter dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
