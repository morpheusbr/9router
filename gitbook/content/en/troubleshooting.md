...
Common issues and solutions when using HiperRouter.
...
1. **Auto-refresh (default):**
   HiperRouter automatically refreshes tokens. Wait 30 seconds and retry.
...
1. **Check if HiperRouter is running:**
   ```bash
   # Check process
   ps aux | grep 9router
